...
>
> ⚠️ **净覆盖率口径统一（173 冻结）**：本文 Step1 核验的 **371/628（一级）、376/623（含嵌套）**（@12:37 快照）均为**【历史口径】**（早于 388 重建）。**【历史快照】当前权威见 `out/coverage.json`：326 success / 673 miss / 999 = 32.63%**（generated_ts 2026-07-03 12:50:24, allow_override=10）。本文「真回写前必先按盘上文件重跑内容 QC」的方法论仍有效。唯一权威 + 待并入项见 **《基线口径冻结说明-388-173.md》**。
