...
>
> 📖 **接手/深入本项目 → 先读 `交接说明书-143.md`**（接手总纲：实现原理与路径 + 全功能使用 + 现状边界 + 下一波 ROI + 坑/经验/认知地图；整合索引全部权威文档）。全文档地图见 `项目索引.md`。
...
### 图示（GitHub 原生渲染 Mermaid）

| 图 | 用途 |
|---|---|
| [`思维导图-项目目标达成-368基线-157.md`](思维导图-项目目标达成-368基线-157.md) | **一眼看懂**目标 / 已实现 / 数据洞察 / 交付 / 下一阶段 |
| [`流程图-一键正门功能步骤-157.md`](流程图-一键正门功能步骤-157.md) | `run_all.py` 端到端管线步骤（输入→定位→下载→QC→命名→coverage） |
| [`架构图-源与模块层次-157.md`](架构图-源与模块层次-157.md) | 编排层→管线→22 源→下载/QC→A5→输出/工具的模块层次 |
