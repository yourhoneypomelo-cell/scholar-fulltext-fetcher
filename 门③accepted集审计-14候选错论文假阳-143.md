...
## 二·终裁、8 条 gold 冲突逐条终裁执行结果（-145，2026-07-04 02:53）

- **逐条开卷全文复核（终裁依据）**：8 条盘上 PDF 逐一重开卷——**自述 DOI 全部跨社他篇、期望 DOI 均不在正文前 3 页、正文题名均与期望不符**（证据同 §二表格）；且**全盘 glob 搜索每个 DOI 的所有副本**，7 条仅此一份、1 条（chemrev.0c00083）另有 batch7/rejected/ 同内容副本——**无任何「真身在别处」的翻案可能**。
- **裁定「gold 误判」而非「事后覆盖」**：8 份文件 mtime 全部为 **07-01 12:26–14:58**（batch4/6 原始下载时刻），**早于** writeback149 gold 裁定（07-03）——当时靠标题吻合免死、未查 PDF 自述 DOI，同题他刊标题重叠高 → 误免死。非并发重跑覆盖。
- **已执行**：① `_writeback149_gold59.txt` 移除 8 条（59→51,原件备份 `out/_writeback149_gold59.bak_pre_gate3adj145_20260704_024813.txt`）;② 7 条追加硬黑 `qc_merge_highconf_wrong.csv`（`verdict=gate3_adj145_gold_misallow`;c3ee44078h 属主已先入硬黑,免重复）;③ 备份 `out/coverage.bak_pre_gate3adj145_20260704_024936.json` → 按权威配方重建。
- **验证**：8 条现全部 `status=miss, qc=hard_reject` ✓;差分「失=恰这 8 条、无其他误伤」✓。
- **新口径（ts 2026-07-04 02:53:08）**：**success = 367 / miss 632 / 36.74%**;qc: hard 44 / soft 157 / allow 52（60−8）/ before-qc 568。**注意两笔并发叠加**：①本终裁 −8;②在飞 `_run147_full`（628 全量重扫,属主发起,当时跑到 354/628）**首次被默认一级扫描收录,其 22 条新命中并入**（before-qc 558→568 的主因,含 openalex_content 多key池新源产出）;净效果 371−8+叠加 = 367。run147 完跑后属主重建时数字将再变,唯 `out/coverage.json` 为准。
- **14 条门③假阳至此全部处置完毕**（6 非白名单 -143 已拉黑 + 8 gold 冲突 -145 本轮终裁）,websearch 同题他刊假阳在 accepted 集的回溯清剿闭环。

## 三、建议处置（分级、防误伤 gold）〔历史,已全部执行〕

1. ~~**P0 安全**：6 条非白名单 → 追加硬黑 → 重建 coverage（368→362）。低风险、门③ 直接证据。~~ ✅ -143 已执行（见 §二·补）。
2. ~~**P1 需裁**：8 条 gold 冲突 → 先逐条开卷全文复核确认后从 gold59 移除 + 拉黑。~~ ✅ -145 已终裁执行（见 §二·终裁）。
3. **纪律（仍有效）**：coverage 是单写者共享文件（V.2）且本仓有并发提交,回写须与单写者/属主对齐、加写锁、`--verify` 自证;数字唯 `out/coverage.json`（U.3）。
