...
3. **《回收结论主表》尚不存在**（仓内 `*主表*.md` 命中 0），**batch7 无独立回收 md**；且更严重的是**头条成功率数字四代漂移**（`最终交付清单`≈82% / `北极星汇总`71.4% / `回收实测结论`44.8% / `本波回收交付汇总`定稿37.9% / **实时口径 37.1%**）。归并《回收结论主表》时必须**先统一口径阶梯、锁定单一权威数字**（见 §三）。
