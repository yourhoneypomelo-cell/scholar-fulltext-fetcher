...
- **采纳 3 条**（在盘净源开卷人核=真全文，属 DOI 键黑名单假阳 → 纠假阳入 `--qc-allow`）：
  ...
  - `10.1016/s1872-2067(17)62899-7`（elsevier，**须从 hard 黑名单移除**）
...
- **净增修正：156 的「建议 +4 / 完整可救 +6」→ 终裁「+3 / 完整可救 +5」**。映射官方 `coverage.json`（success=379/37.9%；立即档 jcou/jechem 2 条交 -151 回写 → 381）：**本终裁 3 条 → 381 → 384（38.4%）**。156「全 6 条 → 385（38.5%）」应修正为「5 条 → 384（38.4%）」。
...
| `10.1016/s1872-2067(17)62899-7` | elsevier | `rerun_elsevier_143/fetch/pdfs/` · openaire · 2,187,222B · 9p | 一致（Chin. J. Catal. 2017） | ✅ 真全文：CJC 38 (2017) 1549–1557「Pd₁/ZnO 单原子催化」，含卷期页/摘要/正文；正文含 `1549`/`single` | **hard**+union+manifest_hard | **采纳** | 入 `--qc-allow` **且从 hard 黑名单移除** |
...
| **完整可救合计** | +6 | **+5** | — |
| 官方 coverage 映射 | 379 →（+2）381 →（+6 全）385 | 379 →（+2）381 →（**+3**）**384（38.4%）** | 剔 1.5053761 |
...
- **立即档 +2**（156 认定、-151 在写/已写）：`10.1016/j.jcou.2022.102356`、`10.1016/j.jechem.2020.06.007`
- **终裁采纳 +3**：`10.1016/j.apcatb.2021.119925`、`10.1016/s1872-2067(17)62899-7`、`10.1002/anie.201406637`
- **`10.1016/s1872-2067(17)62899-7` 额外动作**：在 **hard 黑名单**，回写前须从 hard 名单移除（否则 hard 优先级或压过 qc_allow，属主务必核 `resolve_qc_sets` 取值顺序）。
...
1. **QC verdict 必须对「落盘 PDF」实时正文复核，不能只信 `metadata.jsonl` 下载期记录**。1.5053761 实锤：manifest 记 match/100，落盘却是平台用户指南。建议 `build_coverage` / 预筛脚本在采信 manifest verdict 前，对落盘 PDF 至少抽验一次正文标题 match。
...
*核验 2026-07-02｜谷歌学术人机认证-147｜纯判定 + 文档、未改核心 `.py` / 未回写 coverage / 未重下｜手段：crossref 核标题 + 开卷读落盘 PDF 正文 + pypdf 四处探针｜承 156（4 条建议）、148（A 档 6 候选）｜采纳 3 交 -151 回写（+3→384/38.4%）、驳回 1（1.5053761 真源重取交 route-B）、加固 3 交 -140/-161｜回报总指挥 -144。*
